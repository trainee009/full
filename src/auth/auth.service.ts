...
export class AuthService {
  ...
  async forgotPassword(email: string) {
    ...
    // Store OTP with user email key
    await this.redisClient.set(`otp:${email}`, otp.toString(), {
  ...
  async verifyOTP(dto: VerifyOTPDto) {
    ...
    const cachedOTP = await this.redisClient.get(`otp:${dto.email}`);

    if (cachedOTP !== dto.otp.toString())
    ...
    await this.redisClient.del(`otp:${dto.email}`); // deleting otp here
